...
public class UserEvaluateListActivity extends BaseActivity implements AbsListView.OnScrollListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    private GenericsAdapter mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new GenericsAdapter(UserEvaluateListActivity.this, AppConstants.page_tag_evaluate);
...
//        listView.setAdapter(adapter);
    ...
    @OnClick({R.id.mama_role_container})
    public void doOnClick(View view) {

        switch (view.getId()) {
            case R.id.mama_role_container:

                Intent intent = new Intent(UserEvaluateListActivity.this, UserProfileActivity.class);
...
//        showLoadingProgress();
    ...
    public void onEventMainThread(UserEvaluateEvent event) {
        hideLoadingProgress();
        L.d(OkHttpExecutor.TAG, "onEventMainThread-UserEvaluateListActivity>" + event.getResultStr());

        List<Evaluate> responseData = event.getData().evalList;

        L.d("asker", "--------------" + responseData);

//        adapter.refresh(data);

        data = responseData;

        mAdapter.refresh(AppConstants.page_tag_evaluate, data);
