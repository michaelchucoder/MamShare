...
public abstract class AbsPagerTab extends ViewGroup {
    ...
    protected int mTabTextSize = 15; // tab文字大小
    protected int mTabBackgroundResId = R.drawable.bg_tab_text;// tab背景资源
    protected int mTabTextColorResId = R.color.tab_text_color; // tab文字颜色
    ...
    private int selfWidthSize;


    public static int MODEL_EVEN = 0x000001;
    public static boolean isMoveBroker = false;
    public static int MODEL_MOVEBROKER = 0x000010;
    /**
     * 模式
     */
    public static int MODEL = 0x000000;

    public AbsPagerTab(Context context) {
        this(context, null);
    }

    public AbsPagerTab(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AbsPagerTab(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        // 把一个值从dip转换成px
        mIndicatorHeight = DensityUtil.dp2px(getContext(), mIndicatorHeight);
        mDividerPadding = DensityUtil.dp2px(getContext(), mDividerPadding);
        mTabPadding = DensityUtil.dp2px(getContext(), mTabPadding);
        mDividerWidth = DensityUtil.dp2px(getContext(), mDividerWidth);
        mTabTextSize = DensityUtil.dp2px(getContext(), mTabTextSize);
        // 创建一个scroller
    ...
    private void onViewPagerChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (mViewPager.getAdapter() instanceof IconTabProvider) {// 如果想要使用icon作为tab，则需要adapter实现IconTabProvider接口
                ...
                addPageTab(i, mViewPager.getAdapter().getPageTitle(i).toString(), mTabCount);
            }
        }
    ...
    public abstract void addPageTab(final int position, String title, int len);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        selfWidthSize = MeasureSpec.getSize(widthMeasureSpec) - getPaddingLeft() - getPaddingRight();
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (childLayoutParams.width == LayoutParams.MATCH_PARENT) {
                childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(selfWidthSize, MeasureSpec.EXACTLY);
        ...
        // 不使用本功能
        if (totalChildWidth <= selfWidthSize) {// 如果子Tab的总宽度小于PagerTab，则采用平分模式
            int splitWidth = (int) (selfWidthSize / (mTabCount - goneChildCount + 0.0f) + 0.5f);
            for (int i = 0; i < mTabCount; i++) {
                final View child = getChildAt(i);
                if (child == null || child.getVisibility() == View.GONE) {
                    continue;
            ...
            mMaxScrollX = totalChildWidth - selfWidthSize;
            mSplitScrollX = (int) (mMaxScrollX / (mTabCount - goneChildCount - 1.0f) + 0.5f);
        }

        if (widthMode == MeasureSpec.EXACTLY) {
            mContentWidth = selfWidthSize;
        } else {
            mContentWidth = totalChildWidth;
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {// 这里简化了，没有考虑margin的情况
        if (changed) {
            ...
            int left = l;
            if (totalChildWidth <= selfWidthSize) {
                left += (selfWidthSize - totalChildWidth) / 2;
            }
